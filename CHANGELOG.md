...
- Reworked `detect_concurrency_risks` around bounded, field-sensitive SSA access summaries with goroutine capture tracking, lockset propagation, and atomic awareness.
- Updated documentation to describe the new concurrency analysis model and its conservative limits around unknown roots, containers, reflection, CGO, and unresolved calls.

## [0.6.4] - 2026-05-13

### Performance

- **Decouple AST-Only Tools from go/packages**: Tools that strictly scan syntax (like `list_http_routes`, `list_grpc_endpoints`, `find_duplicate_methods`, and `compute_complexity_metrics`) now bypass `packages.Load` dependency resolution. By using a fast-path `filepath.WalkDir` combined with `parser.ParseFile`, syntax-only queries load approximately 32x faster.
...
- Added `GetOrLoadSyntaxOnly` loader path to prevent unnecessary SSA building and typechecking for AST-only tools (`list_http_routes`, `list_grpc_endpoints`, `find_duplicate_methods`, `compute_complexity_metrics`), drastically reducing memory consumption on large repositories.
- `list_grpc_endpoints` now correctly resolves imports directly from the AST when type information is not available.
...
- Limitations section in README documenting CHA over-approximation, concurrency
  heuristics, SSA scope, and dead code detection caveats.
